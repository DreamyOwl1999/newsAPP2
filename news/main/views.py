...
def post_newsSection(request):
    if request.is_ajax and request.method == "POST":
        if hasattr(request.user.profile, "newssection"):
            print("true")
            form = newsForm(request.POST or None, request.FILES, instance = request.user.profile.newssection)
            if form.is_valid():
                news = form.save()
                print(form.fields['profile_pic'])
                print(request.user.profile.newssection.profile_pic)
        else:
            print("false")
...
def get_pp(request):
    try:
        return JsonResponse({"pp": request.user.profile.newssection.profile_pic.url }, status =200)
    except:
        return JsonResponse({}, status =200)

def delete_pp(request):
    pp  = request.user.profile.newssection
    request.user.profile.newssection.profile_pic.delete()
    pp.save()
    return JsonResponse({}, status =200)
...
def profile_load(request):
    ...
    form.fields['choices'].initial = request.user.profile.newssection.choices
    form.fields['profile_pic'].initial = request.user.profile.newssection.profile_pic
    profpic = request.user.profile.newssection.profile_pic

    try:
      print("picture")
      print(profpic.url)
      return render(request, "main.html", {"form": form, "name": user, "profile_pic":request.user.profile.newssection.profile_pic})
    except:
       print("no picture")
       return render(request, "main.html", {"form": form, "name": user})
